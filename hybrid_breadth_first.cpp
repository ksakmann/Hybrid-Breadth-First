...
vector<HBF::maze_s> HBF::expand(HBF::maze_s state, vector<int> goal,vector< vector<int> > grid) {
  int g = state.g;
  double x = state.x;
  double y = state.y;
  ...
  for(double delta_i = -35; delta_i < 40; delta_i+=5){
    ...
    double delta_x = goal[0] - x2;
    double delta_y = goal[1] - y2;

    double D = 1.0;
    // non-holonomic heuristic ( = does not care about drivability)
    //double h2 = pow(delta_x*delta_x + delta_y*delta_y,0.5);
    // double h2 = max(abs(delta_x),abs(delta_y));
    int ix = idx(x2);
    int iy = idx(y2);

    double obstacle_cost = 1E4;
    if ((iy >= 0 && iy < (int)grid.size()) && (ix >= 0 && ix < (int)grid[0].size()) ){
      if (grid[iy][ix] == 0){
        obstacle_cost = 0.0;        
      }
    }

    double h2 = D * (abs(delta_x) + abs(delta_y));  +  obstacle_cost;

    // TODO: add a holonomic heuristic to penalize wrong orientations
  ...
  // sort the states according to the lowest f value  
  sort( next_states.begin( ), next_states.end( ), [ ]( const maze_s& lhs, const maze_s& rhs )
  {
   return lhs.f < rhs.f;
  }
  );
